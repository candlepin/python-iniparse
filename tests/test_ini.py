...
class test_other_lines(unittest.TestCase):
    def test_empty(self):
        for s in ['asdf', '; hi', '  #rr', '[sec]', 'opt=val']:
            self.assertEqual(ini.empty_line.parse(s), None)
        for s in ['', '  ', '\t  \t']:
            self.assertEqual(str(ini.empty_line.parse(s)), s)

    def test_continuation(self):
        for s in ['asdf', '; hi', '[sec]', 'a=3']:
            self.assertEqual(ini.continuation_line.parse(s), None)
        for s in [' asdfasd ', '\t mmmm']:
            self.assertEqual(ini.continuation_line.parse(s).value,
                             s.strip())
            self.assertEqual(ini.continuation_line.parse(s).to_string(),
                             s.rstrip().replace('\t',' '))
...
"""))
    ...
    def test_invalid(self):
        for (org, mod) in self.inv:
            ip = ini.ini_namespace(StringIO(org), parse_exc=False)
